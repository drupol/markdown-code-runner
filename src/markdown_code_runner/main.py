...
def process_markdown_file(
    ...
    original_lines = filepath.read_text(encoding="utf-8").splitlines()
    md = MarkdownIt()
    tokens = md.parse("\n".join(original_lines))
        ...
        if token.type == "fence" and token.map and token.info:
            ...
            for config_id, lang_config in matching_configs:
                ...
                if should_replace:
                    ...
                    replacement_block = [f"```{lang}"] + output_lines + ["```"]
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(
                ...
                try:
                    ...
                    return (
                        f"ERROR: Unknown placeholder {{{e.args[0]}}} in execute command",
                        True,
                    )
